...
function sevCfg(severity: string) {
  return severityConfig[severity as keyof typeof severityConfig] ?? severityConfig.medium
}

// Incidents — 100% real, straight from the incident store. Active = anything not
// resolved; History = resolved records. Empty store (seed: none) shows empty
// states until real incidents are created by inject scripts / alert webhooks.
export default function IncidentsPage() {
  ...
  const active = apiIncidents
    .filter((i) => i.status && i.status !== "resolved")
  ...
  const totalCritical = active.filter((i) => i.severity === "critical").length
  ...
  return (
    <main className="max-w-[1600px] mx-auto px-4 lg:px-6 py-6 flex flex-col gap-6">
              ...
              <div className="flex items-center gap-3 min-w-0 flex-1">
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <div className="flex items-center gap-4 mt-1.5">
                    ...
                    {(inc.affectedUsers ?? 0) > 0 && (
                      <span className="text-[10px] text-muted-foreground flex items-center gap-1">
                        <Users className="w-2.5 h-2.5" />
                        {(inc.affectedUsers ?? 0).toLocaleString()} affected
                      </span>
                    )}
                  </div>
                </div>
      ...
      {/* Historical archive — resolved, persisted incidents */}
      <div>
